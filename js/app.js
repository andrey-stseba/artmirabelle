(() => {
    ...
    let formValidate = {
        ...
        nameTest(formRequiredItem) {
            return !/^\p{Lu}\p{Ll}*(?:-\p{Lu}\p{Ll}*)?\r?$/u.test(formRequiredItem.value);
        }
    };
    function formSubmit() {
    ...
    let pageId = document.querySelector("[data-id-page]").getAttribute("data-id-page"), navItem = document.querySelector(`[data-id-nav=${pageId}]`);
    if (pageId == navItem.getAttribute("data-id-nav")) navItem.classList.add("_active");
    window["FLS"] = true;
    isWebp();
    spollers();
    formFieldsInit({
